...
class DecisionAgent:
    ...
    def chat(
        ...
        type_hints = {
            "stock_analysis": (
                "\n\n## 当前任务：个股分析\n"
                "用户正在询问一只具体股票。优先使用提供的实时数据做诊断。\n"
                "严格按照个股诊断结构输出：量价数据→技术研判→资金面→综合评分→操作建议→一句话。\n"
                "如果没有实时数据，用盘中候选池中的信息补充。\n"
                "必须有具体的入场价、目标价、止损价。\n"
                "风险提示要具体（如 '日内位置0.85偏高，追高需3%以上回调再入场'）。"
            ),
            "sector_analysis": (
                "\n\n## 当前任务：板块分析\n"
                "用户正在询问板块情况。分析：板块资金流向→强度排名→龙头个股→持续性判断→操作策略。"
            ),
            "market_analysis": (
                "\n\n## 当前任务：大盘研判\n"
                "用户正在询问市场整体情况。分析：市场广度→情绪指标→资金主线→风险等级→仓位建议。"
            ),
            "stock_pick": (
                "\n\n## 当前任务：选股推荐\n"
            ...
            "general": (
                "\n\n## 当前任务：通用咨询\n"
                "回答用户的交易相关问题。有数据时引用数据，无数据时给出通用原则。"
            ),
        }
